...
func (dao *userDao) FindAll() (list []UserModel, err error) {
	db := dao.gm.Find(&list)
	if err = db.Error; db.Error != nil {
		return
	}
...
func (dao *userDao) FindAllWhere(query interface{}, args ...interface{}) (list []UserModel, err error) {
...
func (dao *userDao) Raw(sqlStr string, params ...interface{}) (list []UserModel, err error) {
...
func (dao *userDao) Paginate(offset, limit int) (count int64, list []UserModel, err error) {
	db := dao.gm.Model(&UserModel{}).Count(&count).Offset(offset).Limit(limit).Find(&list)
	if err = db.Error; db.Error != nil {
		return
	}
	return
